"""
Created on Sun May 17 03:07:22 2020
    ...
    def __init__(self, filename, fs=0, nch=0, nbits=0):
        ...
        if filename[-3:] == 'pcm':
            if fs == 0 or nch == 0 or nbits == 0:
                sys.exit("Please provide sampling frequency, number of channels and number of bits per sample for PCM audio file.")
...
class CircBuffer:
    ...
    def ordered(self):
        return np.concatenate((self.sample[self.pos:], self.samples[:self.pos]))
...
class BitStream:
    ...
    def __init__(self, data, nbits, invmsb=False):
        """
            Insert lowest nbits of data in output buffer
        """
        if invmsb:
            data = self.invertmsb(data, nbits)
        datainbytes = self.splitinbytes(data, nbits, self.pos&0x07)
        ind = self.pos // 8
...
def get_scalefactors(sbsamples,sftable):
    """
        Calculate scale factors for subbands. Scale factor is equal to the smallest number in the table greater than all the subband samples in a particular subband. Scalefactor table indices are returned.
    """
