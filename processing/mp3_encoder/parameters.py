"""
Created on Fri May 16 00:17:22 2020
...
class Tables:
    """
        Read all the tables necessary for encoding, including the psychoacoustic model
    """
    ...
    def __init__(self, fs, bitrate):
        ...
        # Read ISO psychoacoustic model 1 table containing critical band rates, absolute thresholds and critical band boundaries
        ...
        half_fft_size = np.uint16(FFT_SIZE/2 + 1)
        self.map = np.zeros(half_fft_size, dtype='uint16')
        for i in range(self.subsize -1):
            for j in range(self.line[i], self.line[i+1]):
                self.map[j] = i
        for j in range(self.line[self.subsize - 1], 257):
            self.map = self.subsize - 1


        
        # SNR table, needed for bit allocation in the ISO psychoacoustic model 1
        self.snr = np.array((0.00, 7.00,  16.00, 25.28,
                            31.59, 37.75, 43.84, 49.89,
                            55.93, 61.96, 67.98, 74.01,
                            80.03, 86.05, 92.01), dtype='float32') 
        ...
        self.qca = np.array(( 0.750000000, 0.875000000, 0.937500000, 0.968750000,
  		                      0.984375000, 0.992187500, 0.996093750, 0.998046875,
  		                      0.999023438, 0.999511719, 0.999755859, 0.999877930,  
  		                      0.999938965, 0.999969482, 0.999984741), dtype='float32')
        self.qcb = np.array((-0.250000000, -0.125000000, -0.062500000, -0.031250000,
  	                         -0.015625000, -0.007812500, -0.003906250, -0.001953125,
  		                     -0.000976563, -0.000488281, -0.000244141, -0.000122070, 
...
class EncoderParameters:
    ...
    def __init__(self, fs, nch, bitrate):
        ...
        if bitrate == 32 and nch == 2:
            sys.exit('Bitrate of 32kb/s is insufficient for encoding of stereo audio')

        self.bitrate = bitrate
        self.nch     = nch
        self.fs      = fs

        if self.fs not in (32000, 441000, 48000):
...
def iso_window():
    ...
    """
    return np.loadtxt('tables/ISOwindoecoeffs', dtype='float32')
